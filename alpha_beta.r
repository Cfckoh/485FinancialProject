...
k<-1 #proportional coefficient affecting Rprime, let it be 1 for now but maybe it must be a function of e and M that makes Rp_max = N?
1/∑∑γ 
...
mark_data<-data.frame(mark_data,Rp, Rp_max) %>% 
  ...
  mutate(bb = case_when(rt > 0 ~ "bull",
                        rt < 0 ~ "bear",
                        TRUE ~ "notbb")) %>%
  mutate(Rp_abs = abs(Rp)) %>% mutate(Rp_absV = TV*Rp_abs)
...
hdbb<-mark_data %>% group_by(bb) %>% summarize(sum(rt_absTV)/sum(TV))
d_p<-hdbb$hd[2]
d_m<-hdbb$hd[1]
d_r<-(d_m - d_p)/2
